...
for i in "${arr[@]}"
do
   echo "./target/release/zmu-cortex-m3 run tests/rustbook/target/thumbv7m-none-eabi/debug/examples/$i"
   timeout 1s ./target/release/zmu-cortex-m3 run tests/rustbook/target/thumbv7m-none-eabi/debug/examples/$i || true
   echo ""
done
