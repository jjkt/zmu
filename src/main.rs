...
use zmu_cortex_m::semihosting::{decode_semihostcmd, SemihostingCommand, SemihostingResponse, SysExceptionReason};
...
pub fn run_bin<T: Bus, R: Bus>(code: &mut T, sram: &mut R) {
    ...
    core.reset();
    while running {
        core.run(|imm32, r0, r1| if imm32 == 0xab {
        ...
        if semihost_triggered {
            ...
            let semihost_response = match semihost_cmd {
                ...
                SemihostingCommand::SysWrite { handle, data } => {
                    println!("SEMIHOST: SYS_WRITE({}, {})", handle, data.len());
